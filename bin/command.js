...
function loadPlugins(source) {
  fs.readdir(source, function(err, files) {
    for(var i =0; i< files.length;i++) {
      ...
      Kajs.plugins[file] = require(path)
    }
  })
}
...
var SWITCHES = [
  ...
  ['-v', '--version',       'display Kajs version'],
...
function version() {
  console.log("Kajs version " + Kajs.VERSION)
  process.exit(0)
}
